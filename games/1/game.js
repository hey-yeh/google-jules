...
class MainScene extends phaser_1.default.Scene {
    ...
    constructor() {
        super({ key: 'MainScene' });
        this.shapeObjects = [];
        this.shapeSize = Math.min(window.innerWidth, window.innerHeight) * 0.2;
        this.padding = 20;
        this.instructionTextObject = null;
        this.currentInstruction = null;
        this.scoreTextObject = null;
    ...
    create() {
        ...
        this.scoreTextObject = this.add.text(20, 20, `Score: ${this.score}`, {
            fontFamily: 'Arial',
            fontSize: '24px',
            color: '#ffffff'
        });
    ...
    displayShapes() {
        ...
        const gameWidth = this.cameras.main.width;
        const gameHeight = this.cameras.main.height;
        let shuffledShapes = phaser_1.default.Utils.Array.Shuffle([...this.shapes]);
        ...
        for (let i = 0; i < 2; i++) { // Rows
            for (let j = 0; j < 2; j++) { // Columns
                ...
                graphics.setInteractive(new phaser_1.default.Geom.Rectangle(-this.shapeSize / 2, -this.shapeSize / 2, this.shapeSize, this.shapeSize), phaser_1.default.Geom.Rectangle.Contains);
                graphics.on('pointerdown', () => this.handleShapeTap(shapeType));
                this.shapeObjects.push(graphics);
            }
        }
    }
    handleShapeTap(shapeName) {
    ...
    gameOver() {
        ...
        // Game Over Text
        const gameOverText = this.add.text(gameWidth / 2, gameHeight / 2 - 100, 'GAME OVER', { fontFamily: 'Arial', fontSize: '48px', color: '#ff0000', align: 'center' }).setOrigin(0.5);
        // Final Score Text
        const finalScoreText = this.add.text(gameWidth / 2, gameHeight / 2 - 30, `You got ${this.score} correct!`, { fontFamily: 'Arial', fontSize: '32px', color: '#ffffff', align: 'center' }).setOrigin(0.5);
        // Try Again Button
        const tryAgainButton = this.add.text(gameWidth / 2, gameHeight / 2 + 50, 'Try Again', {
            fontFamily: 'Arial',
            fontSize: '32px',
    ...
    generateInstruction() {
        const randomShapeIndex = phaser_1.default.Math.Between(0, this.shapes.length - 1);
        const selectedShape = this.shapes[randomShapeIndex];
        const instructionType = phaser_1.default.Math.Between(0, 1) === 0 ? 'POSITIVE' : 'NEGATIVE';
        ...
        if (this.instructionTextObject) {
            ...
            this.instructionTextObject = this.add.text(gameWidth / 2, 50, // Positioned at the top-middle
            instructionString, {
